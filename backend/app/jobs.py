...
async def _analyze_audio_async(analysis_id: str):
    ...
    try:
        ...
        analysis.started_at = datetime.now(timezone(timedelta(hours=3)))
        await analysis.save()
        ...
        analysis.finished_at = datetime.now(timezone(timedelta(hours=3)))
        await analysis.save()
        
        # Update session status to completed
        session.status = "completed"
        session.completed_at = datetime.now(timezone(timedelta(hours=3)))
        await session.save()
